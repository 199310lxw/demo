...
public class CustomizeRecyclerView extends RecyclerView {
    private static String TAG = "CustomizeRecycler";
    private boolean isFristLayout=true;//是不是第一次进入界面
    private static int sHeaderViewMeasuredHeight;//头部view的高度
    private View mHeaderView;//头部view
    private View mFooterView;//底部view

    private RefreashAdapter mRefreashAdapter;

    private float mDownX;//开始触摸屏幕时的x坐标
    private float mDownY;//开始触摸屏幕时的y坐标
    private float mDx;//滑动的x轴距离
    private float mDy;//滑动的Y轴距离
    ...
    public CustomizeRecyclerView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
